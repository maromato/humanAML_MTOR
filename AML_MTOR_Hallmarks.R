...
################ Heatmap with HALLMARK_MTORC1_SIGNALING??? ##################################################
z2 <- y[rownames(y) %in% c[,1],]

heatplot(as.matrix(z2),labRow = FALSE, cexCol=0.4)
dim(z2)
# extract col dendrogram information
sTree<-heatplot(as.matrix(z2), returnSampleTree=TRUE)
plot(sTree)
...
colnames(MTH) <- c("ID","MTOR_Hallmark")
head(MTH)
...
#survival curve
fit <- survfit(Surv(OS, Status) ~ MTOR_Hallmark, data = t,type="kaplan-meier")
ggsurvplot(fit, data= t, legend.labs = c("mTORC low", "mTORC high"),
           pval = TRUE)

pchisq(survdiff(Surv(OS, Status)~MTOR_Hallmark, data=t)$chisq, 1, lower.tail=FALSE)

#p value calculation
p_all <- pchisq(survdiff(Surv(OS, Status)~MTOR_Hallmark, data=t)$chisq, 1, lower.tail=FALSE)# overall
...
p_300 <- pchisq(survdiff(Surv(OS300, Status300)~MTOR_Hallmark, data=t)$chisq, 1, lower.tail=FALSE)# pval at day300
...
p_600 <- pchisq(survdiff(Surv(OS600, Status600)~MTOR_Hallmark, data=t)$chisq, 1, lower.tail=FALSE) # pval at day600
...
p_900 <- pchisq(survdiff(Surv(OS900, Status900)~MTOR_Hallmark, data=t)$chisq, 1, lower.tail=FALSE) #pvalue at day 900

p_total <- rbind(p_all, p_300, p_600, p_900)
colnames(p_total) <- "MTOR_Hallmark"
p_MTOR_Hallmark <- p_total
p_total

write.table(t, "G12417_metadata_MTOR_Hallmark.txt",sep="\t")
write.table(p_total, "G12417_pvalue_MTOR_Hallmark.txt",sep="\t")
...
t[t$MTOR_Hallmark == TRUE,1]


length(t[t$MTOR_Hallmark == TRUE,1])
MTORhigh <- NULL
MTORimlow <- NULL
MTORhigh<- x2[,colnames(x2) %in% t[t$MTOR_Hallmark == TRUE,1]]
...
head(out2)
write.table(out2,"humanAML_MTOR_Hallmark.txt",sep="\t")
head(out2)
...
dim(out3)
head(out3)
write.table(out3,"humanAML_MTOR_Hallmark_p_05.txt",sep="\t")
...
write.table(intersect(toupper(e[,1]),out4$Gene),"human_mouse_MTOR_Hallmark_high_DEG.txt",sep="\t")

#********************************************************************************
#GSEA for mTORC high and low of MTOR_Hallmark
...
write.table(unlist(fgseaRes[order(pval), ]),"GSEA_mouse_human_MTOR_Hallmarks.txt",sep="\t")
