...
z <- y[rownames(y) %in% b[,1],]
heatplot(as.matrix(z),labRow = FALSE, cevCol=0.3)

# extract col dendrogram information
sTree<-heatplot(as.matrix(z), returnSampleTree=TRUE)
...
tbl <- table(mTORhigh, clust.cutree)
...
#survival curve
fit <- survfit(Surv(OS, Status) ~ MTOR_RAP, data = t,type="kaplan-meier")
ggsurvplot(fit, data= t, legend.labs = c("mTORC low", "mTORC high"),
           pval = TRUE)

pchisq(survdiff(Surv(OS, Status)~MTOR_RAP, data=t)$chisq, 1, lower.tail=FALSE)

#p value calculation
p_all <- pchisq(survdiff(Surv(OS, Status)~MTOR_RAP, data=t)$chisq, 1, lower.tail=FALSE)# overall
...
p_300 <- pchisq(survdiff(Surv(OS300, Status300)~MTOR_RAP, data=t)$chisq, 1, lower.tail=FALSE)# pval at day300
...
p_600 <- pchisq(survdiff(Surv(OS600, Status600)~MTOR_RAP, data=t)$chisq, 1, lower.tail=FALSE) # pval at day600
...
p_900 <- pchisq(survdiff(Surv(OS900, Status900)~MTOR_RAP, data=t)$chisq, 1, lower.tail=FALSE) #pvalue at day 900

p_total <- rbind(p_all, p_300, p_600, p_900)
colnames(p_total) <- "MTOR_RAP"
p_MTOR_RAP <- p_total
p_total

write.table(t, "G12417_metadata_MTOR_RAP.txt",sep="\t")
write.table(p_total, "G12417_pvalue_MTOR_RAP.txt",sep="\t")
...
length(t[t$MTOR_RAP == TRUE,1])
MTORhigh <- NULL
MTORimlow <- NULL
MTORhigh<- x2[,colnames(x2) %in% t[t$MTOR_RAP == TRUE,1]]
...
head(out2)
write.table(out2,"humanAML_MTOR_RAP.txt",sep="\t")
head(out2)
...
dim(out3)
head(out3)
write.table(out3,"humanAML_MTOR_RAP_p_05.txt",sep="\t")
